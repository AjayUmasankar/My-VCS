...
# Run legit.pl on a series of commands and
# run the reference implementation (2041 legit) in parallel on the same commands
#
# stop with an error if legit.pl's behaviour differences from the reference implementation

# use some useful functions from Perl core modules
